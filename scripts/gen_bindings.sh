...
rm -f aubio-sys/src/bindings_*.rs

for target in $targets; do
    cp target/$target/debug/build/aubio-sys-*/out/bindings.rs aubio-sys/src/bindings_$(echo $target | sed -r 's/^([^-]+).*$/\1/' | sed 's/i686/x32/' | sed 's/x86_64/x64/').rs
done
